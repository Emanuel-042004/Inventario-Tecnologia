...
<script>
    ...
    window.location.replace('{{ route('impresoras.historial.index', $impresora->id) }}');
});
</script>
@endif
...
<div class="container mt-4">
    <h1 style="color: black;"><strong>Historial de la impresora: {{ $impresora->serial }}</strong></h1>
    ...
    <div class="row">
        ...
        <div class="col-md-6" style="margin-top: 35px;">
            ...
            <form action="{{ route('impresoras.historial.store', $impresora->id) }}" method="POST">
                ...
                <button type="submit" class="btn btn-danger shadow" style="margin-bottom: 30px;">Agregar Historial</button>
            </form>
        </div><br><br>

        <!-- Tabla de Historial en la parte derecha -->
        <div class="col-md-6">
        @if(count($historialImpresora) > 0)
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="{{ route('impresoras.historial.edit', ['impresora' => $impresora->id, 'historialImpresora' => $registro->id]) }}"
                                class="btn btn-secondary shadow">Editar</a>
                            <form
                                action="{{ route('impresoras.historial.destroy', ['impresora' => $impresora->id, 'historialImpresora' => $registro->id]) }}"
                                method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
                                    class="btn btn-danger eliminar-historial-impresora shadow">Eliminar</button>
                            </form>
                        </td>
                    </tr>
                    @endforeach
            ...
            <div class="sticky-btn-container"
                onclick="window.location.href='{{ route('mantenimientos.index', ['tipo' => 'impresoras', 'id' => $impresora->id]) }}'">
