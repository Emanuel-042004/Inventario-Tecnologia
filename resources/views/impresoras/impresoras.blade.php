    ...
    <div class="col-12">
            ...
            <form action="{{ route('impresoras.index') }}" method="GET">
                <div class="form-group float-end">
                    ...
                    <select class="form-control" name="filtro" id="filtro">
                        ...
                        <option value="propias" {{ Request::get('filtro')==='propias' ? 'selected' : '' }}>Propios
                        </option>
                        <option value="alquiladas" {{ Request::get('filtro')==='alquiladas' ? 'selected' : '' }}>
                            Alquilados</option>
                    </select>
        ...
        <div>
            <a href="#" class="btn btn-dark shadow" data-bs-toggle="modal"
                data-bs-target="#agregarImpresoraModal">Agregar Impresora</a>
        </div>
                ...
                <div class="card mb-4 shadow h-100 d-flex flex-column">
                    <div class="card-header mb-4" style="background-color: rgb(204, 35, 35)">
                        <h4 class="card-title"><strong>Cod Interno:</strong> {{$impresora->serial}}
                            <a href="{{ route('historiales.index', ['tipo' => 'impresoras', 'id' => $impresora->id]) }}"
                    ...
                    <div class="card-body">
                       ...
                       @can('impresoras.edit')
                        <a href="" class="btn btn-dark shadow" data-bs-toggle="modal"
                            data-bs-target="#editarImpresoraModal{{ $impresora->id }}" style="border-radius: 50px;">
                        ...
                        <!--BOTON ELIMINAR -->
                        <form action="{{ route('impresoras.destroy', $impresora->id) }}" class="d-inline shadow"
                            method="POST">
                            @csrf
                            @method('DELETE')
                            <button type="button" class="btn btn-danger eliminar-impresora float-end"
